...
\begin{proof}
	...
	Since $(a/d)^n$ divides $a^n$ and is coprime to $b^r$, it must divide $u$.
	Therefore, $d^n = b^r v$, where $v$ is an integer multiple of $u$.

	Lemma~\ref{lemma-1} implies that there exists a prime factor $p$ of $a$, and $C' > 0$ depending
	only on $a$ and $b$, such that $\nu_p(v) \ge C' n$.
	But $u$ is a multiple of $v$, hence $\nu_p(u) \ge C' n$.
	Therefore, $c_b(u) > C \log n$ for some $C > 0$, by Theorem~\ref{thm:cbn}.

	Finally, $c_b(u) = c_b(a^n)$, since $u$ and $a^n$ have
...
In contrast with the situation for $a^n$, where prime-power divisibilty
...
\section{Digital sums of powers: the general case} \label{general-case}
...
\begin{proof}
	...
	\begin{equation*} \label{m-ratio}
		\frac{m(i+1)}{m(i)} < C_5 \log n, \qquad C_5 = 2C_4.
	\end{equation*}

	In either case, we have
	\[
		\frac{m(i+1)}{m(i)} < C_6 \log n, \qquad C_6 = \max(C_2, C_5).
	\]
	...
	\begin{equation} \label{estimate-3}
		\log m(k) < (k - 1) (\log \log n + C_{7}), \qquad C_7 = \log C_6.
	\end{equation}
