...
This sequence grows very rapidly. Now, define another sequence by adding
...
This sequence of digital sums grows much more slowly, and it is not monotonic.
...
But first, we review the relevant notation and terminology.
...
The \emph{floor} of a real number $x$,
...
\[
	C g(n) < |f(n)| < C' g(n) \quad \text{for all } n \in D.
\]
...
Since $2^{14} > 10^4$, any number divisible by $2^{14}$ % and less than $10^{14}$
must be at least $10^4$.
Thus the last $14$ digits of $2^n$,
\[
2^n \bmod 10^{14},
\]
...
\begin{theorem} \label{thm:digit-sum-bound-2-to-n}
	...
	Suppose that $N$ is divisible by $2^{e_{k}}$ but not $10$.
	Then $c_{10}(N) \ge k$.
\end{theorem}
...
\begin{proof}
	...
	Therefore,
	$c_{10}(a^n) \ge k \ge \log_4 n$, by Theorem~\ref{thm:digit-sum-bound-2-to-n}.
\end{proof}
...
\begin{theorem} \label{thm:multiples-of-powers}
	...
	Suppose that $N$ is divisible by $a^{e_{k}}$ but not $b$.
	Then $c_b(N) \ge k$.
\end{theorem}

\begin{proof}
	Follow the proof of Theorem~\ref{thm:digit-sum-bound-2-to-n}, but replace $2$
...
The following lemma will permit us to disregard trailing zeros in the base-$b$
expansion of $a^n$.
...
\begin{proof}
	...
	Therefore, $\displaystyle t > p^{C_1 n}$, which implies that
	\[
		\log t > C_1 n \log p.
	\]
...
Our proof relies on a version of Baker's theorem on lower bounds for
linear forms in logarithms, which we state without proof.
...
In our application of Baker's theorem, $\alpha_1, \ldots, \alpha_n$ are rational integers.

{\bf Remark:} A \emph{rational integer} means an integer in the usual sense,
that is, an element of $\mathbb{Z}$. This is to be distinguished from an
\emph{algebraic integer}, which is a root of monic polynomial
...
\begin{proof}
	...
	Fix $i\in \{1,\dots,k-1\}$. Our goal is to show that large
	gaps between digit positions cannot occur, by showing that
	\[
		\frac{m_{i+1}}{m_i} < C \log n
	\]
	for some $C>0$. This will imply that the number of gaps,
	...
	\[
		a^n = b^{m - m_i} q + r.
	\]
	In other words, the digits up to the $i$-th nonzero digit
	form an integer $q$, and the digits from the $(i+1)$-st
	nonzero digit onward form an integer $r$.
	...
	We apply Baker's theorem to obtain a lower bound on $|\Lambda|$.
	This will imply an upper bound on the gap $m_{i+1}/m_i$,
	as shown in \eqref{ineq:gap-bound} below.
	There are two cases, depending on the size of $q$.

		{\bf Case 1:} $q < b^2$, or equivalently, $m_i \le 2$.

	In this case, we can apply Baker's theorem to \eqref{def:Lambda}
	with $A \gets b^2$ and $B \gets n$, giving
	\[
		|\Lambda| > C_1^{-\log n}
	\]
	...
	In this case, we can apply Baker's theorem
	to \eqref{def:Lambda} with
	$A \gets q$ and $B \gets n$, giving
	\[
	...
	\begin{equation*} \label{m-ratio}
		\frac{m_{i+1}}{m_{i}} < C_5 \log n, \quad C_5 = 2C_4.
	\end{equation*}
	...
	Thus $m_k \asymp \log t$, and $\log t \asymp n$ by Lemma~\ref{lem:digit-position-growth}, so
...
This is a long and complicated proof, so let's review the main steps.
\begin{enumerate}
...
{\bf Use of AI tools.}
The authors used ChatGPT (OpenAI) for editorial assistance, including
improving clarity of exposition and checking the logical flow of arguments.
All mathematical results, proofs, and final wording are the authors' own,
