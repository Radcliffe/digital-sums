...
In a foundational paper from 1973, Senge and Straus~\cite[Theorem 3]{senge-straus1973}
...
first we establish a structural digit-sum bound for integers forced
to be divisible by powers of $a$ but not by $b$, and then we apply it to $a^n$
itself under an additional irrationality condition.
...
\begin{proof}
	...
	with $a$ where appropriate, and replace $10$ with $b$.
\end{proof}
...
We conjecture that $s_{b}(n!) \asymp n \log n - n$ and $s_b(\Lambda_n) \asymp n$,
...
\begin{proof}
	...
	\[
		|\Lambda| > C_1^{-\log n} = n^{-C_2}
	\]
	for some constant $C_2 > 0$ depending only on $a$ and $b$.
	Thus,
	\[
		a^{-n} r > n^{-C_2},
	\]
	...
	\[
		m(i+1) < C_3 \log n
	\]
	for some constant $C_{3}>0$ depending only on $a$ and $b$.
	
	Since $m(i) \ge 1$, we also have
	\[
		\frac{m(i+1)}{m(i)} < C_3 \log n.
	\]
	...
	\[
		|\Lambda| > C_4^{-\log q \log n}
	\]
	for some $C_4>1$ depending only on $a$ and $b$. Thus,
	\[
		a^{-n} r > C_4^{-\log q \log n},
	\]
	and hence, for $n$ sufficiently large,
	\begin{equation}
		\frac{-\log(a^{-n} r)}{\log(q)} < C_5 \log n
		\label{estimate-2}
	\end{equation}
	...
	\[
		\frac{m(i+1)}{m(i)} < C_6 \log n.
		\label{m-ratio}
	\]
	for some $C_6 > 0$ depending only on $a$ and $b$.

	In either case, we have
	\[
		\frac{m(i+1)}{m(i)} < C_7 \log n, \qquad C_7 = \max(C_3, C_7).
	\]
