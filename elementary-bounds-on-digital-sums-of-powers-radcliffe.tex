...
\begin{document}
	...
	\section{Digital sums of powers of two}
		...
		We will present an informal proof that $c_{10}(2^{n})$ tends to infinity as $n \to \infty$.
		...
		\begin{proof}
			...
			the integer $r$ is also divisible by $2^{\varepsilon(k-1)}$. 
		...
		{\bf Exercise.} Show that every power of 3 has a multiple $m$, not divisible by $10$,
		such that $c_{10}(m) = 2$.
	...
	\section{Generalizing to other bases}
		...
		\begin{proof}
			...
			the integer $r$ is also divisible by $p^{\varepsilon(k-1)}$. 
		...
		\begin{theorem}
			...
			Then $c_{b}(a^{n}) \ge \log_r n$ for all $n \ge 1$, where $r \ge 2$ is an integer
		...
		We will replicate Stewart's result in Section \ref{general-case}.
	...
	\section{Related sequences}
		...
		depending only on $b$,
		and they established the same inequality for $s_{b}(\Lambda_n)$.
	...
	\section{Digital sums of powers: the general case}
		...
		\begin{lemma}
			...
			Then there exists $C$ and $C'$, depending only on $a$ and $b$, 
			such that whenever
			\[
			 	a^n = b^r u \qquad \text{with } b \nmid u,
			\]
			we have
			\[
				C n \le \log u \le C' n.
		...
		\begin{proof}
			...
			\end{equation*}
			where $m = \lfloor \log_b a \rfloor$, $k = c_b(a^n)$, 
			$d_i \in \{1, \ldots, b-1\}$,
			and
			\[
			...
			Combining \ref{estimate-1} and \ref{estimate-2} gives
			\begin{equation}
			...
			By Lemma~\ref{lemma}, $\log m(k) = \log n + O(1)$, 
			so \eqref{estimate-3} implies			
			\[
	...
	{\bf Exercise.} Prove that the number of nonzero decimal digits in the $n$th
