...
\begin{document}
	...
	\section{Generalizing to other bases}
		...
		\begin{proof}
			Follow the proof of Theorem \ref{base-ten-lower-bound}, but replace $2$ with $p$
			where appropriate, and replace $10$ with $b$.
		\end{proof}

		It is desirable to relax the condition that $b$ does not divide $n$,
		...
		In 2025, S. Jaiswal (personal communication) proved that if $a, b \ge 2$ are integers,
	...
	\section{Digital sums of factorials and LCMs}
		...
		In this section, we demonstrate a logarithmic lower bound
		for digital sums of factorials, and also for the digital sum
		of $\Lambda_n := \lcm(1, 2, \ldots, n)$.

		We need the following elementary result, which is due to
		Stolarsky~\cite{stolarsky1980} for base two,
		and generalized to all bases by Balog and Dartyge~\cite{balog2012}.
	...
	\section{Digital sums of powers: the general case}
		...
		\begin{proof}
			...
			\[
				\nu_p(a) \nu_q(b) > \nu_q(a) \nu_p(b).
			\]
			Hence $\xi(a)>0$ for the function $\xi=\xi_{b,p,q}$ defined in Equation \eqref{xi-notation}.
