...
\begin{document}
	...
	\section{Notation}
		...
		so we restrict our attention to $c_{b}(n)$. 
		...
		We use Bachmann-Landau notation\cite{knuth1997} to describe the approximate size of functions.
	...
	\section{Digital sums of powers of two}
		...
		\begin{proof}
			...
			\[
				n = 10^{e(k-1)} q + r,
			\]
			where $q \ge 0$ and $0 \le r < 10^{e(k-1)}$.
			

			Since $n \ge 2^{e(k)} > 10^{e(k-1)}$, the quotient $q$ is at least $1$.

			Since $n$ and $10^{e(k-1)}$ are divisible by $2^{e(k-1)}$, 
			the remainder $r$ is also divisible by $2^{e(k-1)}$. 
	...
	\section{Generalizing to other bases}
		...
		\begin{theorem}
			...
			prime divisor of $b$. Let $(e(k))_{k\ge1}$ be a sequence of integers such
			that $e(1) \ge 1$ and $p^{e(k)} > b^{e(k-1)}$ for all $k \ge 2$. 
			If $\nu_p(n) \ge e(k)$ and $b \nmid n$, then $c_{b}(n) \ge k$.
		...
		\begin{proof}
			...
			\[
				n = b^{e(k-1)}q + r,
			\]
			where $q \ge 0$ and $0 \le r < b^{e(k-1)}$.

			Since $n \ge p^{e(k)}> b^{e(k-1)}$, the quotient $q$ is at least $1$.

			Since $n$ and $b^{e(k-1)}$ are divisible by $p^{e(k-1)}$, 
			the remainder $r$ is also divisible by $p^{e(k-1)}$. 
		...
		It is desirable to relax the condition that $b \nmid n$,
		since multiplying by a factor of the form $b^r$ does not change the number of nonzero digits
		of $n$ in base $b$. To facilitate this, we introduce the following notation.
		Let $b \ge 2$ be an integer with distinct prime divisors $p$ and $q$.
		Define the function $\xi = \xi_{b,p,q}$ by
			\[
				\xi(n) = \nu_{p}(n) - \nu_{q}(n) \frac{\nu_{p}(b)}{\nu_{q}(b)}.
			\]
		One easily verifies that $\xi(b^r u) = \xi(u)$ for all integers $r \ge 0$ and $u \ge 1$.
		Intuitively, $\xi$ is a modified version of $\nu_p$
		that is insensitive to trailing zeros in the base-$b$ expansion of its argument.

		\begin{theorem}
			Let $b \ge 2$ be an integer that is not a power of a prime, let $p$ and $q$
			be distinct prime divisors of $b$, and let 
			$(e(k))_{k\ge1}$ be defined as in Theorem \ref{general-powers-lower-bound}.
			If $\xi(n) \ge e(k)$, then $c_{b}(n) \ge k$. 
			In particular, $\lim_{n\to\infty} c_{b}(a_n) = \infty$
		...
		\begin{proof}
			...
			and let $e(k) = r^{k-1}$ for $k \ge 1$.
			This sequence satisfies $e(1) \ge 1$ and
			\[
				p^{e(k)} > b^{e(k-1)}
			\]
			for all $k \ge 2$.
			...
			so that
				$r^{k-1} < \xi(a^n) \le r^k$.
			Then $\xi(a^n) > e(k)$, so Theorem \ref{phi-lower-bound} implies that $c_{b}(a^n) \ge k$.
		...
		In 2025, Jaiswal\cite{jaiswal2025} proved that if $a \ge 2$ and $b \ge 2$ are positive integers,
		$a$ divides $b$, and if $b$ has at least one prime divisor that does not divide $a$,
		then \[
			c_{b}(a^n) > \frac{\log n}{\log \left(2\log b / \log a\right)}
		\]
	...
	\section{Digital sums of factorials and LCMs}
		...
		We give two examples here: factorials, and the least common multiple
		...
		\begin{theorem}
			...
			\[
				(p - 1) \nu_{p}(b) \ne (q-1) \nu_{q}(b).
			\]
			Then $c_{b}(n!) > C \log n$ for some $C > 0$ depending only on $b$.
			In particular, $c_{b}(n!) \to \infty$ as $n \to \infty$.
		\end{theorem}
		...
		\begin{theorem}
			...
			that is not a power of a prime. Then $c_{b}(\Lambda_n) > C \log \log n$
			for some $C > 0$ depending only on $b$. In particular, $c_{b}(\Lambda_n) \to \infty$
			as $n \to \infty$.
		\end{theorem}
	...
	\section{Another elementary bound on digital sums}

	We present another elementary bound on digital sums, due to
	Stolarsky\cite{stolarsky1980} for base two, and generalized to
	all bases by Balog and Dartyge\cite{balog2012}.

	\begin{theorem}
		Let $r \ge 1$ and $b \ge 2$ be integers.
	...
	\begin{proof}
		...
		so that
		\[
			m = \sum_{i=0}^{k-1} B_i b^{ri}, \qquad (0 \le B_i < b^r).
		\]
	...
	This result implies lower bounds of the form $s_b(n!) > C \log n$
	and $s_b(\Lambda_n) > C \log n$ for some $C > 0$, depending only on $b$.
	For if $n \ge b^r - 1$, then $n!$ and $\Lambda_n$ are divisible by $b^r - 1$,
	hence $s_b(n!) \ge (b-1)r$ and $s_b(\Lambda_n) \ge (b-1)r$.
...
arXiv preprint arXiv:2401.12345 (2016).
