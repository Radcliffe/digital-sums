...
\begin{document}
	...
	\section{Notation}
		...
		% This notation can be combined with other operations. For example,
		% \[
		% 	f(n) = g(n) + o(h(n))
		% \]
		% means that there exists a function $k$ such that
		% \[
		% 	f(n) = g(n) + k(n) \qquad \text{and } k(n) = o(h(n)).
		% \]
	...
	\section{Digital sums of powers: the general case}
		...
		\begin{proof}
			...
			\end{equation*}
			where $m = \lceil \log_b a \rceil$, $k = c_{b}(a^n)$, 
			$d_i \in \{1, \ldots, b-1\}$,
			and
			\[
