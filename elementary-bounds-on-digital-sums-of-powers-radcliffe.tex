...
\begin{document}
	...
	\section{Digital sums of powers of two}
		...
		This shows that $2^n$ increases to infinity with $n$.
	...
	\section{Generalizing to other bases}
		...
		define a function $\xi = \xi_{b,p,q}$ as follows:
			\[
				\xi(n) = \nu_{p}(n) - \nu_{q}(n) \frac{\nu_{p}(b)}{\nu_{q}(b)}.
			\]
		It is easy to verify that $\xi(b^r m) = \xi(m)$ for all $r \ge 0$
		and $m \ge 1$. Intuitively, $\xi$ is a modified $\nu_p$
	...
	\section{Related sequences}
		...
		depending only on $b$,
		and he established the same inequality for $s_{b}(\Lambda_n)$.
	...
	\section{Digital sums of powers: the general case}
		...
		expansion of $a^n$ increases to infinity with $n$, provided that $\log(b)/\log(a)$
		is irrational. This result has been proven before \cite{senge-straus1973,stewart1980},
		but our exposition should be accessible to a wider audience.

		The condition is necessary, because if $\log(a)/\log(b) = r/s \in \mathbb{Q}$, then
		$a^{ns} = b^{nr}$ for all $n$, so $a^{ns}$ has only one nonzero digit in its base-$b$
		expansion.

		Our proof relies on a deep result in transcendental number theory, which will not be proved here.
		...
		\begin{theorem} \cite[p. 23]{baker1975}
			...
			have heights at most $A' \ge 4$ and $A \ge 4$ respectively. Let $\beta_1, \ldots, \beta_n$
			be rational integers with heights (absolute values) at most $B \ge 2$.
			If $\Lambda = \beta_1 \log \alpha_1 + \cdots + \beta_n \alpha_n \ne 0$,
			there exists $C > 1$, depending only on $A$, $A'$, $B$, $n$, and $d$,
			such that $|\Lambda| > C^{-\log A \log B}$.
			\label{baker}
		\end{theorem}

		Recall that a nonzero algebraic number $\alpha$ is a root of an irreducible polynomial $P$
		with integer coefficients. We may assume that the greatest common divisor of the coefficients
		of $P$ is 1, and the leading coefficient is positive. These requirements uniquely determine $P$.
		The \emph{degree} of $\alpha$ is equal to the degree of $P$, and the \emph{height} of $\alpha$
		is the maximum of the absolute values of the cofficients of $P$. For a rational integer, the degree
		is $1$ and the height is the absolute value.

		% Let $a, b \ge 2$ be integers. If $\log(a)/\log(b) = r/s \in \mathbb{Q}$,
		% then the sequences $(s_b(a^n))_{n\ge1}$ and $(c_b(a^n))_{n\ge1}$ are periodic,
		% since $a^s = b^r$. So we will assume for the remainder of this section that
		% $\log(a)/\log(b)$ is irrational.

		We will need the following lemma, in case some power of $a$ is divisible by $b$.
		\begin{lemma}
			Let $a, b \ge 2$ be integers such that $\log(a)/\log(b)$ is irrational.
			Then there exists a constant $C > 0$ so that if $a^n = b^r m$ and $m \nmid d$
			then $\log m > Cn$.
			\label{lemma}
		\end{lemma}

		\begin{proof}
			If $\log(a)/\log(b)$ is irrational then there exist primes $p, q$ so that
			$\nu_p(a) \nu_q(b) > \nu_q(a) \nu_p(b)$. Consequently, $\xi(a) > 0$, where
			$\xi = \xi_{b,p,q}$ is defined as in Section 3.

			Since $\nu_p(m) \ge \xi(m) = \xi(a^n) = n \xi(a)$, it follows that
			$m$ is divisible by $p^{\lceil n \xi(a)\rceil}$, 
			so $\log m > Cn$ where $C = \xi(a) \log p > 0$.
		\end{proof}
		...
		\begin{theorem} 
			...
			Then $c_b(a^n)$ increases to infinity with $n$.
		\end{theorem}

		\begin{proof}
			Let
			\begin{equation*}
				a^n = b^m (d_i b^{-m(1)} + \cdots + d_r b^{-m(r)}),
			\end{equation*}
			where $m = \lfloor \log_b a \rfloor$, $r = c_b(a^n)$, $d_i \in \{1, \ldots, b-1\}$,
			and $1 = m(1) < \cdots < m(r) \le m$. Assume that $r \ge 2$.

			Choose $i \in \{1, \ldots, r-1\}$. We seek an upper bound on $m(i+1)/m(i)$,
			which will imply an upper bound on $n$.

			Let $q = b^{m(i)} (d_1 b^{-m(1)} + \cdots + d_i b^{-m(i)})$ and
			$r = b^m (d_{i+1} b^{-m(i+1)} + \cdots d_r b^{-m(r)})$, so that
			$a^n = b^{m - m(i)} q + r$. 

			% Let $q = \lfloor b^{m-m_i} a^n \rfloor$ and $r = a^n \bmod b^{m-m_i}$, so that
			% $a^n = b^{m-m_i} q + r$.
			...
			\begin{align*}
			    b^{m-1} &< a^n < b^m, \\
                b^{m(i)-1} &< q < b^{m(i)}, \\
				b^{m-m(i+1)} &< r < b^{m-m(i+1)+1}.
			\end{align*}

			By combining these inequalities, we find that
			\begin{equation}
				-\log(a^{-n} r) / \log(q) \approx m(i+1) / m(i).
				\label{estimate-1}
			\end{equation}

			Let $\Lambda = -n \log a + (m - m(i)) \log b + \log q$.
			Then $|\Lambda| = -\log(1 - a^{-n} r) \approx a^{-n} r$.
			By Theorem \ref{baker}, $|\Lambda| > C^{-\log n \log q}$ for some $C > 1$
			depending only on $a$ and $b$. Therefore, $a^{-n} r > C^{-\log n \log q}$,
			so
			\begin{equation}
				-\log(a^{-n} r) / \log(q) < \log n + C.
				\label{estimate-2}
			\end{equation}

			Since $\log m_r = \sum_{i=1}^{r-1} \log(m(i+1)/m(i))$, 
			Equations \ref{estimate-1} and \ref{estimate-2} imply that
			\begin{equation}
			   m(r) < (r - 1) (\log \log n + C).
			   \label{estimate-3}
			\end{equation}

			But $m(r) = \Theta(\log n)$ by Lemma \ref{lemma}, hence
			\[
			r > \frac{\log n}{\log \log n + C}
			\]
			for some $C > 0$ depending only on $a$ and $b$, when $n$ is sufficiently large.
			Since $r = c_b(a^n)$, this confirms that $\lim_{n\to\infty} c_b(a^n) = \infty$.
		...
		% In this section, we will prove that $\lim_{n\to\infty} c_{10}(3^n) = \infty$.
		% This proof is decidedly \emph{not} elementary; it requires a deep result
		% from transcendental number theory.

		% Our initial proof that $\lim_{n\to\infty} c_{10}(2^n) = \infty$
		% was based on the idea that the decimal expansion of $2^n$ cannot contain
		% long strings of consecutive zeros in certain locations.
		% But this is not true for $3^n$.
		% In fact, the decimal expansion of $3^n$ can end in $000\ldots1$ for
		% any number of zeros, since $3^{4\cdot 10^k} \equiv 1 \pmod{10^{k+1}}$
		% by Euler's totient theorem. So a different approach is required.
